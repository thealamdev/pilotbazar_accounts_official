<div class="pt-7">
    ...
    <header class="pb-4 d-flex justify-content-between align-items-center">
        <div class="">
            <p class="fs-5 fw-600">Model Year Update</p>
            <span>Please must fill the field where (*) sign is visible.</span>
        </div>
        <div>
            <a href="{{ route('admin.version1.vehicle-management.vehicle.module.model-year.index') }}" class="bg-transparent border border-slate-400 px-4 py-1 rounded" type="button">Model Year</a>
        </div>
    </header>
    <!-- Header Part End !-->
    ...
    <div class="border border-slate-400 rounded p-3">
        <form action="#" wire:submit="update" method="POST" wire:key="model_year_update_component">
            @method('put')
            @csrf
            <div class="row">
                <div class="col-lg-6">
                    <label for="form.name">Model Year <span class="text-danger">*</span></label>
                    <div class="input-group mb-3">
                        <span class="input-group-text"><i class="fa-solid fa-droplet"></i></span>
                        <input type="text" wire:model.live="form.name" value="{{ $response?->name }}" class="form-control @error('form.name')
                            {{ 'is-invalid' }}
                        @enderror" placeholder="color name">
                        @error('form.name')
