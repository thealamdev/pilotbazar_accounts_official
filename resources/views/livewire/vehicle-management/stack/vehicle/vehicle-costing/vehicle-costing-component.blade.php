        ...
        <div class="border border-slate-400 rounded p-3 mt-5">
            <form action="#" wire:submit="saveMediaCost" method="POST">
                ...
                @for ($i = 1; $i <= $mediaCostFormLen; $i++)
                    <div class="row" wire:key={{ $i }}>
                        <div class="col-lg-4">
                            <label for="form.costing_field">Client Name<span class="text-danger">*</span></label>
                            <div class="input-group mb-3">
                                <span class="input-group-text">৳</span>
                                <input type="text" wire:model="form.client_name[]" class="form-control @error('form.client_name') {{ 'is-invalid' }} @enderror" placeholder="client name">
                        ...
                        <div class="col-lg-4">
                            ...
                            <div class="input-group mb-3">
                                <span class="input-group-text">৳</span>
                                <input type="number" wire:model="form.amount[]" class="form-control @error('form.amount') {{ 'is-invalid' }} @enderror" placeholder="amout">
                                @error('form.amount')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="input-group mb-3">
                                <textarea wire:model="form.remarks[]" cols="30" rows="3" class="form-control" placeholder="remakrs"></textarea>
                            </div>
                        </div>
                    </div>
                @endfor
            ...
            <form wire:submit.prevent="saveMediaCost">
                ...
                @for ($i = 0; $i < $mediaCostFormLen; $i++)
                    <div class="row" wire:key="media-costing-form-{{ $i }}">
                        <div class="col-lg-4">
                            <label for="form.client_name.{{ $i }}">Client Name<span class="text-danger">*</span></label>
                            <div class="input-group mb-3">
                                <span class="input-group-text">৳</span>
                                <input type="text" wire:model.live="form.client_name.{{ $i }}" class="form-control @error('form.client_name.' . $i) is-invalid @enderror" placeholder="client name">
                                @error('form.client_name.' . $i)
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
